        ...
        {loading ? (
          <div className="flex flex-col gap-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="rounded-2xl border border-border/40 bg-card p-6 sm:p-8 animate-pulse"
                style={{ height: "160px" }}
              />
            ))}
          </div>
        ) : experiences.length === 0 ? (
          <div className="text-center py-16 text-muted-foreground">No experience entries available.</div>
        ) : (
          /* Timeline Stack */
