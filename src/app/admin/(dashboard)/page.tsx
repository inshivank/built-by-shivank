...
// Derive stats from existing content files (static until Phase 3 DB integration)
const totalProjects = featuredProjects.length;
const featuredCount = featuredProjects.filter((p) =>
  ["Marketplace", "Cyber Threat", "Hate Speech"].includes(p.title)
).length;
const completedCount = featuredProjects.filter(
  (p) => p.status === "Completed"
).length;
const ongoingCount = featuredProjects.filter(
  (p) => p.status === "Ongoing"
).length;
